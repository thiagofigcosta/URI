/* ALUNO: 201612040233 − Thiago Figueiredo Costa
					URI ERROR
*/
#ifndef NCELL_H
#define NCELL_H
typedef struct ncell{
    ...
    */
    int value;
    int i;
    static struct ncell* newCell(int value,int i){//CONSTRUCTOR HERE
    ...
    void print(){
        ...
        if(type==STACK){
            ...
            for(int i=0;i<size;i++){
                stack->push(nCell::newCell(aux->value,aux->i));
                aux=aux->nxt;
            }
            stack->type=QUEUE;
...
int main(){
    int n, m;
    #if (!URI)
    cout<<"URI ERROR, (WA100%),\n 
    		BUT CODE IS RIGTH"<<endl;
    while(1){
    #endif
    cin>>n>>m;
    if(n==m)
        ...
        nQueue *numbers=nQueue::newQueue();
        nCell *cell;
        int c;
        int i;
        numbers->push(nCell::newCell(n,1));
        while(1){
            cell=numbers->pop();
            c=cell->value;
            i=cell->i;
            if(c*2==m)
                ...
                numbers->push(nCell::newCell(c*2,i+1));
                 if(c*3==m)
                    break;
                else{
                    numbers->push(nCell::newCell(c*3,i+1));
                    if(c/2==m)
                        break;
                    else{
                        numbers->push(nCell::newCell(c/2,i+1));
                        if(c/3==m)
                            ...
                            if(c+7==m)
                                ...
                                numbers->push(nCell::newCell(c+7,i+1));
                                if(c-7==m)
                                    break;
                                else{
                                    numbers->push(nCell::newCell(c-7,i+1));
